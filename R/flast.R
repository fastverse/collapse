  ...
  if(is.null(TRA)) {
    ...
    if(is.atomic(g)) {
      if(use.g.names) {
        ...
        lev <- attr(g, "levels")
        res <- .Call(C_flast,x,length(lev),g,na.rm)
        names(res) <- lev
        return(res)
      }
    ...
    if(use.g.names) {
      res <- .Call(C_flast,x,g[[1L]],g[[2L]],na.rm)
  ...
  if(is.null(TRA)) {
    ...
    if(is.atomic(g)) {
      if(use.g.names) {
        if(!is.nmfactor(g)) g <- qF(g, na.exclude = FALSE)
        lev <- attr(g, "levels")
        res <- .Call(C_flastm,x,length(lev),g,na.rm,FALSE)
    ...
    if(!is_GRP(g)) g <- GRP.default(g, return.groups = use.g.names, call = FALSE)
    if(use.g.names) {
      res <- .Call(C_flastm,x,g[[1L]],g[[2L]],na.rm,FALSE)
