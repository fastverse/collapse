...
This short vignette focuses on using *collapse* with the popular *sf*
package by Edzer Pebesma. It shows that *collapse* supports easy
manipulation of *sf* data frames, at computation speeds far above
*dplyr*.

*collapse* v1.6.0 added internal support for *sf* data frames by having
most essential functions (e.g., `fselect/gv`, `fsubset/ss`, `fgroup_by`,
`findex_by`, `qsu`, `descr`, `varying`, `funique`, `roworder`, `rsplit`,
`fcompute`, …) internally handle the geometry column.
...
Computing summary statistics on *sf* data frames automatically excludes
the ‘geometry’ column:
...
We can select columns from the *sf* data frame without having to worry
about taking along ‘geometry’:
...
frame. Keeping the full ‘agr’ attribute is not problematic for all
practical purposes, but not changing ‘bbox’ upon subsetting may lead to
too large margins when plotting the geometries of a subset *sf* data
frame.

One way to to change this is calling
[`st_make_valid()`](https://r-spatial.github.io/sf/reference/valid.html)
on the subset frame; but
[`st_make_valid()`](https://r-spatial.github.io/sf/reference/valid.html)
is very expensive, thus unless the subset frame is very small, it is
better to use `[`,
[`base::subset()`](https://rdrr.io/r/base/subset.html) or
[`dplyr::filter()`](https://dplyr.tidyverse.org/reference/filter.html)
in cases where the bounding box size matters.

## Aggregation and Grouping

The flexibility and speed of
[`collap()`](https://fastverse.org/collapse/reference/collap.md) for
aggregation can be used on *sf* data frames. A separate method for *sf*
objects was not considered necessary as one can simply aggregate the
geometry column using
...
*sf* data frames can also be grouped and then aggregated using
...
Typically most of the time in aggregation is consumed by
[`st_union()`](https://r-spatial.github.io/sf/reference/geos_combine.html)
so that the speed of *collapse* does not really become visible on most
datasets. A faster alternative is to use *geos* (*sf* backend for planar
geometries) or *s2* (*sf* backend for spherical geometries) directly:
...
[`st_make_valid()`](https://r-spatial.github.io/sf/reference/valid.html),
may need to be invoked to ensure valid *sf* object output. Functions
[`collap()`](https://fastverse.org/collapse/reference/collap.md) and
[`fsummarise()`](https://fastverse.org/collapse/reference/fsummarise.md)
are attribute preserving but do not give special regard to geometry
columns.
...
quickly be aggregated using
[`ffirst()`](https://fastverse.org/collapse/reference/ffirst_flast.md)
or [`flast()`](https://fastverse.org/collapse/reference/ffirst_flast.md)
to aggregate the geometry:
...
![plot of chunk
AREA_Ag](https://raw.githubusercontent.com/fastverse/collapse/master/vignettes/figure/AREA_Ag-1.png)

## Indexing

*sf* data frames can also become [*indexed
frames*](https://fastverse.org/collapse/reference/indexing.html)
...
Functions
[`funique()`](https://fastverse.org/collapse/reference/funique.md) and
`roworder[v]()` ignore the ‘geometry’ column in determining the unique
values / order of rows when applied to *sf* data frames.
[`rsplit()`](https://fastverse.org/collapse/reference/rsplit.md) can be
used to (recursively) split an *sf* data frame into multiple chunks.
...
The default in
[`rsplit()`](https://fastverse.org/collapse/reference/rsplit.md) for
data frames is `simplify = TRUE`, which, for a single LHS variable,
would just split the column-vector. This does not apply to *sf* data
frames as the ‘geometry’ column is always selected as well.
...
*sf* data frames can be combined using
[`rowbind()`](https://fastverse.org/collapse/reference/rowbind.md),
which, by default, preserves the attributes of the first object.
...
For transforming and computing columns,
[`fmutate()`](https://fastverse.org/collapse/reference/ftransform.md)
and `ftransform[v]()` apply as to any other data frame.
...
Special attention to *sf* data frames is afforded by
[`fcompute()`](https://fastverse.org/collapse/reference/ftransform.md),
which can be used to compute new columns dropping existing ones - except
for the geometry column and any columns selected through the `keep`
argument.
...
can be used to efficiently convert *sf* data frames to standard data
frames, *data.table*’s or *tibbles*, and the result can be converted
back to the original *sf* data frame using
...
The easiest way to strip a geometry column off an *sf* data frame is via
the function
[`atomic_elem()`](https://fastverse.org/collapse/reference/extract_list.md),
which removes list-like columns and, by default, also the class
attribute. For example, we can create a *data.table* without list column
using
...
This is also handy for other functions such as
[`join()`](https://fastverse.org/collapse/reference/join.md) and
[`pivot()`](https://fastverse.org/collapse/reference/pivot.md), which
are class agnostic like all of *collapse*, but do not have any built-in
logic to deal with the *sf* column.
...
Since v2.0.13, *collapse* explicitly supports/preserves *units* objects
through dedicated methods that preserve the ‘units’ class wherever
sensible.
...
*collapse* provides no deep integration with the *sf* ecosystem and
cannot perform spatial operations, but offers sufficient features and
flexibility to painlessly manipulate *sf* data frames at much greater
speeds than *dplyr*.

This requires a bit of care by the user though to ensure that the
returned *sf* objects are valid, especially following aggregation and
subsetting.
